...
assert GROQ_KEY, "Set GROQ_API_KEY in .env"
...
# Initialize Groq chat clients
creative_llm = ChatGroq(model="llama-3.1-8b-instant", temperature=0.9, groq_api_key=GROQ_KEY)
supervisor_llm = ChatGroq(model="llama-3.1-8b-instant", temperature=0.2, groq_api_key=GROQ_KEY)
...
Constraints:
- Eae**
- **Max 20 words**ch message must be **one complete sentenc
...
5. **No brand names**
6. **No quote marks**
7. IMPORTANT: No quote marks (neither single ' nor double ")

Instructions:
- Otherwise, rewrite as one **benefit-focused**, **professional sentence** (max 20 words).
- Never include quote marks. Always make sure to remove all quotation marks
- Never Print Validation Criteria or Instructions in the output.
...
example_messages = response.content.strip()
print(f"\nFinal Extracted Example Messages (used for all users):\n{example_messages}\n")

# Step 2: Define LangGraph node functions
def generate_message(state: MarketingState) -> MarketingState:
    prompt = generate_prompt.format(**state)
    output = creative_llm.invoke([
        ("system", "You are a telecom assistant generating one engaging marketing line."),
        ("user", prompt)
    ])
    print(f"Generated candidate message:\n{output.content.strip()}\n")
    return {**state, "candidate": output.content.strip()}

def supervisor_check(state: MarketingState) -> MarketingState:
    prompt = supervisor_plan_prompt.format(line=state["candidate"])
    decision = supervisor_llm.invoke([
        ("system", "You are the supervisor deciding whether to accept or regenerate a marketing message."),
        ("user", prompt)
    ])
    print(f"Supervisor LLM decision:\n{decision.content.strip()}\n")
    match = re.search(r"action:\s*(accept|regenerate)", decision.content.lower())
...
def validate_message(state: MarketingState) -> MarketingState:
    ...
    output = supervisor_llm.invoke([
        ("system", "You are validating a telecom marketing message for Indian audiences."),
        ("user", prompt)
    ])
    print(f"Final validated message:\n{output.content.strip()}\n")
    return {**state, "final_message": output.content.strip()}

def evaluate_message(state: MarketingState) -> MarketingState:
    prompt = evaluate_prompt.format(line=state["final_message"])
    evaluation = supervisor_llm.invoke([
        ("system", "You are evaluating the quality of a marketing message on a scale of 1 to 5."),
        ("user", prompt)
    ])
    print(f"Evaluation output:\n{evaluation.content.strip()}\n")
    match = re.search(r"score:\s*([1-5])", evaluation.content)
    ...
    return {
        **state,
        ...
        "evaluation_reason": evaluation.content.strip()
    }
...
workflow.add_conditional_edges("supervisor_check", lambda s: s["decision"], {
    "accept": "validate",
    "regenerate": "generate"
})
workflow.add_edge("validate", "evaluate")
workflow.add_conditional_edges("evaluate", lambda s: ("generate" if s["evaluation_score"] < 4 else "finish"), {
    "generate": "generate",
    "finish": "__end__"
})
...
mlflow.set_experiment("telecom_marketing_with_groq")
mlflow.langchain.autolog()

# Step 5: Run per-customer LangGraph executions
messages = []
...
for _, row in df.iterrows():
    ...
    with mlflow.start_run(run_name=f"customer_{row.customer_id}"):
        ...
        # Log examples and prompts safely
        mlflow.log_dict({"examples": inputs["examples"]}, "input_examples.json")
        ...
        mlflow.log_dict({"evaluation_reason": result["evaluation_reason"]}, "evaluation_reason.json")
...
# Step 6: Update database with final messages
conn = mysql.connector.connect(
    host="localhost", user="root", password="", database="telecom_data"
)
cursor = conn.cursor()
